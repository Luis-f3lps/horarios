...
// COLOQUE O SEU JSON GIGANTE AQUI NOVAMENTE
...
async function main() {
  ...
  for (const [nomeDisciplina, alocacoes] of Object.entries(dadosDisciplinas)) {
    for (const alocacao of alocacoes) {
      ...
      // 1. Limpa o texto do JSON (tira acentos, quebras de linha e deixa minúsculo)
      const textoJson = alocacao.turma.toLowerCase()
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        .replace(/\n/g, ' ');

      // 2. Busca o "DNA" da turma no Banco de Dados
      for (const turmaDb of turmasDb) {
        const textoDb = turmaDb.nome.toLowerCase()
          .normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        
        // Extrai o número do período/ano (ex: acha o "1" em "1º ANO")
        const numeroJson = textoJson.match(/\d+/)?.[0];
        const numeroDb = textoDb.match(/\d+/)?.[0];
        
        // Verifica as letras A e B para os técnicos
        const temA = textoJson.includes(' a') || textoJson.includes(' a\n') || textoJson.includes('- a');
        const temB = textoJson.includes(' b') || textoJson.includes(' b\n') || textoJson.includes('- b');
        
        const dbTemA = textoDb.includes(' a ');
        const dbTemB = textoDb.includes(' b ');

        let letraBate = true;
        if (temA && !dbTemA) letraBate = false;
        if (temB && !dbTemB) letraBate = false;

        // Tradutor de Abreviações do seu JSON específico
        let cursoBate = false;
        if (textoJson.includes('agropec') && textoDb.includes('agropecuaria')) cursoBate = true;
        else if (textoJson.includes('agroind') && textoDb.includes('agroindustria')) cursoBate = true;
        else if (textoJson.includes('info') && textoDb.includes('informatica')) cursoBate = true;
        else if (textoJson.includes('sist') && textoDb.includes('sistemas')) cursoBate = true;
        else if (textoJson.includes('med. vet') && textoDb.includes('veterinaria')) cursoBate = true;
        else if (textoJson.includes('alim') && textoDb.includes('alimento')) cursoBate = true;
        else if (textoJson.includes('flor') && textoDb.includes('florestal')) cursoBate = true;
        else if (textoJson.includes('biol') && textoDb.includes('biologicas')) cursoBate = true;
        else if (textoJson.includes('fisic') && textoDb.includes('fisica')) cursoBate = true;
        else if (textoJson.includes('matema') && textoDb.includes('matematica')) cursoBate = true;
        else if (textoJson.includes('quim') && textoDb.includes('quimica')) cursoBate = true;
        else if (textoJson.includes('pedag') && textoDb.includes('pedagogia')) cursoBate = true;
        else if (textoJson.includes('meio amb') && textoDb.includes('meio ambiente')) cursoBate = true;

        // Se o Número, a Letra e o Curso baterem, achamos a turma exata!
      ...
      if (professorId && turmaId) {
        ...
        logErros.push(`⚠️ Falha ao vincular: Prof '${alocacao.professor}' (Encontrado: ${!!professorId}) | Turma '${alocacao.turma.replace(/\n/g, ' ')}' (Encontrado: ${!!turmaId})`);
      }
    }
  }
  ...
  if (disciplinasParaInserir.length > 0) {
    ...
    console.log(`\n✅ SUCESSO! ${disciplinasParaInserir.length} disciplinas foram salvas no banco com a busca inteligente.`);
  }

  if (logErros.length > 0) {
    console.log(`\n⚠️ ${logErros.length} vínculos falharam por nomes muito fora do padrão (você pode ignorar por enquanto e focar no algoritmo):`);
  }
}
